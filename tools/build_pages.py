...
AFFILIATE_TRACKING_PIXELS = {
    ...
    "https://t.afi-b.com/visit.php?a=y7404W-D7355634_J&p=b984781I":
...
CHART_TOOLTIP_JS = """<script>
(function(){
  var tip=document.createElement('div');tip.className='chart-tooltip';tip.setAttribute('role','status');tip.style.display='none';document.body.appendChild(tip);
  function place(cx,cy){
    tip.style.display='block';
    var w=tip.offsetWidth,h=tip.offsetHeight,pad=14,x=cx+pad,y=cy+pad;
    if(x+w>window.innerWidth-6)x=cx-w-pad; if(x<6)x=6;
    if(y+h>window.innerHeight-6)y=cy-h-pad; if(y<6)y=6;
    tip.style.left=x+'px';tip.style.top=y+'px';
  }
  function hide(){tip.style.display='none';}
  document.addEventListener('mousemove',function(e){
    var a=e.target.closest?e.target.closest('.chart-pt'):null;
    if(!a){hide();return;}
    var t=a.getAttribute('data-tip'); if(!t){hide();return;}
    tip.textContent=t; place(e.clientX,e.clientY);
  });
  document.addEventListener('mouseleave',hide);
  // タッチ端末: 点をタップした瞬間に名称を表示（その後リンク遷移）
  document.addEventListener('touchstart',function(e){
    var a=e.target.closest?e.target.closest('.chart-pt'):null;
    if(!a)return;
    var t=a.getAttribute('data-tip'); if(!t)return;
    var to=e.touches&&e.touches[0]; if(!to)return;
    tip.textContent=t; place(to.clientX,to.clientY);
...
SERIES = [
    ...
    },
    {
        "slug": "nissho-boki",
        "prefix": "日商簿記",
        "title": "日商簿記検定 1〜3級の受験料・合格率一覧",
        "h1": "日商簿記検定とは？｜1級・2級・3級の受験料・合格率・違い",
        "lead": (
            "<p><strong>日商簿記検定</strong>は、日本商工会議所・各地商工会議所が"
            "実施する簿記の検定です。企業のお金の流れを記録・集計する簿記の知識を"
            "測り、就職・転職や経理実務で広く評価されます。<strong>3級</strong>"
            "（商業簿記の基礎）、<strong>2級</strong>（商業簿記＋工業簿記）、"
            "<strong>1級</strong>（会計のプロレベル）の3段階が中心です。</p>"
            "<p>下表で各級の受験料・合格率・実施頻度を比較できます。級ごとの試験科目・"
            "勉強時間の目安・受験資格などは各級の詳細ページをご覧ください。金額・日程は"
            "改定されることがあるため、出願前に必ず公式サイトでご確認ください。</p>"),
        "desc": ("日商簿記検定1級・2級・3級の受験料・合格率・実施頻度を一覧で比較。"
                 "級ごとの違いや各級の詳細ページへのリンクをまとめています。"),
        "faqs": [
            ("日商簿記の3級・2級・1級はどう違いますか？",
             "3級は商業簿記の基礎、2級は商業簿記に工業簿記・原価計算が加わり、"
             "1級は会計基準や会計学まで含む最上位級です。就職では2級以上が"
             "評価されやすいとされます。"),
            ("日商簿記の受験料はいくらですか？",
             "統一試験の受験料は級により異なります（3級・2級・1級で段階的に高くなります）。"
             "正確な金額は本ページの一覧および各級の詳細ページをご確認ください。"),
            ("日商簿記はいつ受けられますか？",
             "2級・3級は統一試験が年3回のほか、テストセンターでのネット試験（CBT）も"
             "随時実施されています。1級は年2回です。最新の日程は公式サイトでご確認ください。"),
        ],
    },
    {
        "slug": "lettering-ginou-kentei",
        "prefix": "レタリング技能検定",
        "title": "レタリング技能検定（レタリング検定）の級・受験料一覧",
        "h1": "レタリング技能検定とは？｜1〜4級の受験料・難易度",
        "lead": (
            "<p><strong>レタリング技能検定</strong>（レタリング検定）は、公益財団法人"
            "国際文化カレッジが実施する（文部科学省後援）、文字のデザイン＝レタリングの"
            "技能を測る検定です。<strong>4級から1級</strong>まであり、"
            "デザイン・広告・印刷などの分野で文字表現の基礎力を証明できます。</p>"
            "<p>下表で各級の受験料・実施頻度を比較できます。級ごとの試験内容・受験資格"
            "などは各級の詳細ページをご覧ください。金額・日程は改定されることがあるため、"
            "出願前に必ず公式サイトでご確認ください。</p>"),
        "desc": ("レタリング技能検定（レタリング検定）1〜4級の受験料・実施頻度を"
                 "一覧で比較。級の構成と各級の詳細ページへのリンクをまとめています。"),
        "faqs": [
            ("レタリング技能検定にはどんな級がありますか？",
             "4級・3級・2級・1級の4段階があり、数字が小さい級ほど高度な文字デザインの"
             "技能が求められます。"),
            ("レタリング技能検定の受験料はいくらですか？",
             "級によって異なり、上位級ほど高くなります。正確な金額は本ページの一覧および"
             "各級の詳細ページをご確認ください。"),
        ],
    },
    {
        "slug": "kensaku-gijutsusha-kentei",
        "prefix": "検索技術者検定",
        "title": "検索技術者検定 1〜3級の受験料一覧",
        "h1": "検索技術者検定とは？｜1級・2級・3級の受験料・難易度",
        "lead": (
            "<p><strong>検索技術者検定</strong>は、情報科学技術協会（INFOSTA）が"
            "実施する、情報検索（データベース検索・情報調査）の知識と技能を測る検定です。"
            "<strong>3級から1級</strong>まであり、図書館・調査・研究開発などの分野で"
            "情報を的確に探し出すスキルを証明できます。</p>"
            "<p>下表で各級の受験料・実施頻度を比較できます。級ごとの試験内容・受験資格"
            "などは各級の詳細ページをご覧ください。金額・日程は改定されることがあるため、"
            "出願前に必ず公式サイトでご確認ください。</p>"),
        "desc": ("検索技術者検定1級・2級・3級の受験料・実施頻度を一覧で比較。"
                 "級の構成と各級の詳細ページへのリンクをまとめています。"),
        "faqs": [
            ("検索技術者検定にはどんな級がありますか？",
             "3級・2級・1級の3段階があり、3級は入門的な内容、上位級ほど高度な"
             "情報検索・調査の技能が求められます。"),
            ("検索技術者検定の受験料はいくらですか？",
             "級によって異なります。正確な金額は本ページの一覧および各級の詳細ページを"
             "ご確認ください。"),
        ],
    },
]
...
def build_series_pages(indexable):
    ...
    for s in SERIES:
        ...
        single = len(groups) == 1
        # 合格率が1件でもある系列だけ合格率列を出す（無い系列は列自体を省く）
        show_pass = any(pass_rate_display(r["pass_rate"]) for r in members)
        head = ('<thead><tr><th scope="col">級・区分</th>'
                '<th scope="col">受験料</th>'
                + ('<th scope="col">合格率</th>' if show_pass else "")
                + '<th scope="col">実施頻度</th></tr></thead>')
        listing = ""
        for gname, rs in groups.items():
            ...
            for r in rs:
                ...
                pr = pass_rate_display(r["pass_rate"]) or "—"
                pcell = f'<td>{esc(pr)}</td>' if show_pass else ""
                body_rows += (
                    f'<tr><th scope="row"><a href="../c/{r["slug"]}.html">'
                    f'{esc(r["name"])}</a></th>'
                    f'<td>{esc(fee)}</td>{pcell}<td>{freq}</td></tr>')
            head_html = (f'<h2 class="hub-grp">{esc(gname)}</h2>'
                         if gname and not single else "")
            listing += (
                f'{head_html}<table class="spec series-table">{head}'
...
.block-all-certs .filter-field label{display:block;font-size:var(--text-table);color:var(--muted);margin-bottom:6px;font-weight:600}
...
.all-certs-table thead th{text-align:left;padding:11px 14px;background:#edf2f8;color:var(--ink);font-weight:var(--fw-semibold);font-size:1rem;border:1px solid var(--table-border);border-bottom:1px solid #9fb4d0;white-space:nowrap}
...
.dm-title{fill:var(--ink-deep);font-size:12.5px;font-weight:600}
.dm-quad{fill:var(--muted);font-size:11.5px;font-weight:600;opacity:.85}
...
.chart-plabel{fill:var(--ink-deep);font-size:11px;font-weight:700;paint-order:stroke;stroke:#fff;stroke-width:3px;stroke-linejoin:round;pointer-events:none}
...
.chart-tooltip{position:fixed;z-index:60;pointer-events:none;background:var(--ink-deep);color:#fff;font-size:var(--text-sm);font-weight:600;padding:6px 10px;border-radius:6px;max-width:min(280px,80vw);box-shadow:0 4px 14px rgba(0,0,0,.28);line-height:1.45}
...
.stats-hensa-table{width:100%;border-collapse:collapse;background:#fff;border:1px solid var(--table-border);font-size:var(--text-table);table-layout:auto}
.stats-hensa-table th,.stats-hensa-table td{padding:9px 12px;border:1px solid var(--table-border);text-align:left;vertical-align:middle;line-height:1.5}
...
/* Spec table (detail) */
table.spec{width:100%;border-collapse:collapse;background:#fff;border:1px solid var(--table-border);font-size:var(--text-table);font-weight:var(--fw-regular);color:var(--ink)}
...
table.spec th,table.spec td{text-align:left;padding:10px 14px;border:1px solid var(--table-border);vertical-align:middle;line-height:1.55}
...
.container:has(.page-article){max-width:880px}
...
.article-card-title a{color:var(--ink-deep);text-decoration:none}
.article-card:hover .article-card-title a{text-decoration:underline}
.article-card-desc{font-size:var(--text-sm);color:var(--muted);line-height:1.7;margin:.5em 0 0;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
...
.article-foot-note{font-size:var(--text-sm);color:var(--muted);margin-top:28px;line-height:1.7}
.article-answer{background:#eef4fb;border:1px solid var(--gray-200);border-radius:var(--radius);padding:14px 16px;margin:18px 0}
...
.article-toc-h{font-weight:700;font-size:var(--text-sm);margin:0 0 6px}
...
.article-table thead th{background:#edf2f8;color:var(--ink);font-weight:var(--fw-semibold);white-space:nowrap;font-size:1rem;border-bottom:1px solid #9fb4d0}
...
.page-article .article-related{border-top:none;padding-top:0}
...
.sage-table{width:100%;border-collapse:collapse;background:#fff;border:1px solid var(--table-border);font-size:var(--text-table)}
.sage-table th,.sage-table td{padding:8px 12px;border:1px solid var(--table-border);text-align:left;vertical-align:middle}
...
.vs-table thead th{background:#edf2f8;color:var(--ink-deep);text-align:center;font-size:1rem;border-bottom:1px solid #9fb4d0}
...
table.cmp thead th{background:#edf2f8;color:var(--ink-deep);font-weight:700;font-size:1rem;border-bottom:1px solid #9fb4d0}
...
.detail-intro-media--affiliate{align-self:stretch;display:flex;flex-direction:column;height:auto}
.detail-intro-media--affiliate .detail-intro-aff{display:block;flex:1 1 auto;align-self:stretch;height:100%;min-height:188px;line-height:0}
.detail-intro-media--affiliate img{width:280px;height:100%;min-height:188px;object-fit:cover;border:none;background:transparent;border-radius:10px}
...
@media(max-width:720px){.detail-intro-row{flex-direction:column-reverse;gap:16px}.detail-intro-media,.detail-intro-media img,.cert-cover,.detail-intro-media--affiliate .detail-intro-aff{width:100%}.detail-intro-media img,.cert-cover,.detail-intro-media--affiliate img{height:190px;min-height:190px}}
